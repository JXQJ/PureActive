...
namespace PureActive.Queue.Hangfire.IntegrationTests
{
    ...
    public class QueueHangfireIntegrationTests : TestBaseLoggable<QueueHangfireIntegrationTests>
    {
        ...
        /// <autogeneratedoc />
        [Fact]
        public void QueueHangfire_Standalone_JobQueue()
        {
            ...
            using (var server = new BackgroundJobServer())
            {
                ...
                var jobIdString = BackgroundJob.Enqueue(() =>
                    Console.WriteLine($"Enqueued Task ID: {Guid.NewGuid().ToStringNoDashes()}"));
                jobIdString.Should().NotBeNull();
                int.TryParse(jobIdString, out var jobId).Should().BeTrue();
                jobId.Should().BeGreaterThan(0);

                var jobStatus =
                    JobQueueClient.WaitForJobToComplete(monitoringApi, jobIdString, 10 * 1000, CancellationToken.None);
